...
class LFM:
    ...
    def __init__(self, data, ratio, F=5, N=2, alpha=0.02, lamda=0.01, topk=10):
        ...
        self.item_ids = set(self.data.ix[:, 1]) # 训练数据中商品的列表
    ...
    def Recommend(self, user):
        ...
        """
        过滤掉用户已经看过或者买过的物品后，计算用户对剩下物品的喜欢程度并返回前n个感兴趣的物品
        Calculate all item user have not meet before and return the top n interest items.
        """
        other_item_ids = self.item_ids ^ self.ui[user]
        predictList = [self.lfmPredict(self.P, self.Q, user, item) for item in other_item_ids]
        series = pd.Series(predictList, index=other_item_ids)
...
if __name__ == "__main__":   
    ...
    #导入数据 
    data=pd.read_csv('./ml-latest-small/ratings.csv',nrows=10000,header=None)
    data=data.drop(0)
    data=data.ix[:,0:1]
